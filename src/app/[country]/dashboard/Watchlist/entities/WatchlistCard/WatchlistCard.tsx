...
const WatchListCard = ({ card }: ICard) => {
    ...
    const handleDelete = async () => {
        try {
            const response = await fetch(`/api/listings/${card.id}`, {
                method: 'DELETE',
            });
            if (!response.ok) {
    ...
    return (
        <div className='tw-max-w-[370px] tw-w-full tw-static' style={{ border: "1px solid #ECECEC", borderRadius: "10px" }}>
            <CarCardsSlider handleDelete={handleDelete} height='260px' imglinks={card.imgs} deletes={true} />
            <div className='tw-flex tw-flex-col tw-gap-[20px] tw-p-[30px]'>
                ...
                <ButtonMain href={'/listing/' + card.slug} text={"Bid Now"} icon={"/images/dashboard/icons/auction.png"} classNames={"tw-gap-2"} onClick={() => setClick((e) => !e)} color={isClick ? "grey" : "blue"} />
            </div>
        </div>
    )
}
